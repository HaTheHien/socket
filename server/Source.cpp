	...
	while (Login == true)
	{
		...
		if (byteReceive > 0)
		{
			...
			if (cat == UPLOAD)
			{
				...
				if (flag == true)
				{
					...
					if (Upload(clientSocket, path))
					{
						vector<string> b;
						blockJSON.lock();
						Mjson.addDocument(pathForJson, SIZE, username, b);
			...
			if (cat == DOWNLOAD)
			{
				...
				path = ROOT + path;
				block.unlock();
				/*string law = Mjson.get(path, username);
				if (law == "")
				{
					send(clientSocket, ("0019" + string(NOTOK) + "can't law download file").c_str(), 20, 0);
					continue;
				}*/
				ifstream fin(path);
				if (!fin.is_open())
...
{
	blockJSON.lock();
	bool flag = DownLoad(clientSocket, PACKG);
	blockJSON.unlock();
	return flag;
}